...
import { Q, fetchSupplierList, fetchPayments, fetchPaymentStats, authHeaders } from './api'
import './Payments.css'
...
function PaymentForm({ suppliers, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/purchase-entries?supplier_id=${supplierId}&limit=50`, { credentials: 'include', headers: authHeaders() })
      .then(r => r.ok ? r.json() : { entries: [] })
      .then(j => setInvoices(j.entries ?? []))
      .catch(() => setInvoices([]))
    // Fetch balance for overpayment guard
    fetch(`${API_URL}/api/suppliers/${supplierId}/balance`, { credentials: 'include', headers: authHeaders() })
